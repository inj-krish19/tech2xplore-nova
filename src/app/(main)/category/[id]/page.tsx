...
export default async function CategoryPage({
    ...
    const [{ items: posts }, recentAuthors] = await Promise.all([
        listPostsByCategory(categoryId, page, PAGE_SIZE),
        recentAuthorsByCategory(categoryId),
    ]);

    return (
        <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-10">
            <p className="font-mono-kicker text-muted-foreground">Category</p>
            <h1 className="mt-1 font-display text-2xl font-semibold">{category.name}</h1>
