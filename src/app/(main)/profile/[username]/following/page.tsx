...
import { getBloggerByUsername, listFollowing } from "@/lib/services/user-service";
...
export default async function FollowingPage({ params }: { params: Promise<{ username: string }> }) {
    ...
    const following = await listFollowing(blogger.authorid).catch(() => []);
    ...
    return (
        <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 sm:py-10">
            ...
            <h1 className="mt-1 font-display text-2xl font-semibold">Following ({following.length})</h1>

            {following.length === 0 ? (
                <p className="mt-8 text-sm text-muted-foreground">Not following anyone yet.</p>
            ) : (
